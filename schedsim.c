...
{ 
  ...
  int i;
	...
	for (i = 0; i < n; i++)
	{
		remain_burst_time[i] = plist[i].bt;
      plist[i].wt = 0; 
	}
...
{ 
  ...
  int remain_burst_time[n];
   ...
   while(j < n){
      int min_index = 0;
      for(i = 1; i < n; i++){
        if (remain_burst_time[i] < remain_burst_time[min_index]){
          min_index = i;
        }
      }

    remain_burst_time[min_index] -= 1;

   if (remain_burst_time[min_index] == 0){
        j += 1;
        min = curr_time + 1;
        plist[min_index].wt = min - plist[min_index].bt;
        remain_burst_time[min_index] = 9999;
      }
    // Increment time lap by one.
    curr_time += 1;
    }
